...
import org.elasticsearch.action.bulk.BulkShardProcessor2;
...
public class UpsertByIdTask2 extends JobTask {
    ...
    public void start() {
        if (node.items().size() == 1) {
            ...
            UpsertByIdNode2.Item item = node.items().get(0);
            ...
            for (UpsertByIdNode2.Item item : node.items()) {
    ...
    private void executeUpsertRequest(final UpsertByIdNode2.Item item, final SettableFuture futureResult) {
        ...
        ShardUpsertRequest2 upsertRequest = new ShardUpsertRequest2(
                shardId,
                node.dataTypes(),
                node.updateColumns(),
                node.insertColumns(),
        ...
        upsertRequest.add(0, item.id(), item.row(), item.version(), item.routing());
    ...
    private List<ListenableFuture<TaskResult>> initializeBulkShardProcessor(Settings settings) {
        bulkShardProcessor = new BulkShardProcessor2(
                clusterService,
                settings,
    ...
    private void createIndexAndExecuteUpsertRequest(final UpsertByIdNode2.Item item,

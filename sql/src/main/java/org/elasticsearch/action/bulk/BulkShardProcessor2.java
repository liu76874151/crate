...
import io.crate.executor.transport.ShardUpsertRequest2;
import io.crate.executor.transport.ShardUpsertResponse;
import io.crate.executor.transport.TransportShardUpsertAction2;
 ...
 */
public class BulkShardProcessor2 {

    private final ClusterService clusterService;
    private final TransportShardUpsertAction2 transportShardUpsertActionDelegate;
    private final TransportCreateIndexAction transportCreateIndexAction;
    private final boolean autoCreateIndices;
    private final int bulkSize;
    private final Map<ShardId, ShardUpsertRequest2> requestsByShard = new HashMap<>();
    ...
    public BulkShardProcessor2(ClusterService clusterService,
                               Settings settings,
                               TransportShardUpsertAction2 transportShardUpsertActionDelegate,
    ...
    public boolean add(String indexName,
        ...
        partitionRequestByShard(indexName, id, row, routing, version);
        executeIfNeeded();
        return true;
    }
    ...
    private void partitionRequestByShard(String indexName,
        ...
        try {
            executeLock.acquire();
            ShardUpsertRequest2 updateRequest = requestsByShard.get(shardId);
            if (updateRequest == null) {
                updateRequest = new ShardUpsertRequest2(
                        shardId,
                        dataTypes,
                        updateColumns,
                        insertColumns,
    ...
    private void executeRequests() {
        try {
            executeLock.acquire();
            for (Iterator<Map.Entry<ShardId, ShardUpsertRequest2>> it = requestsByShard.entrySet().iterator(); it.hasNext(); ) {
                Map.Entry<ShardId, ShardUpsertRequest2> entry = it.next();
                ShardUpsertRequest2 shardUpsertRequest = entry.getValue();
    ...
    private void execute(ShardUpsertRequest2 updateRequest) {
    ...
    private void doRetry(final ShardUpsertRequest2 request, final boolean repeatingRetry) {
        trace("doRetry");
        if (repeatingRetry) {
            try {
    ...
    private void processFailure(Throwable e, ShardUpsertRequest2 shardUpsertRequest, boolean repeatingRetry) {
    ...
    class ResponseListener implements ActionListener<ShardUpsertResponse> {
        ...
        protected final ShardUpsertRequest2 shardUpsertRequest;

        public ResponseListener(ShardUpsertRequest2 shardUpsertRequest) {
    ...
    class RetryResponseListener extends ResponseListener {
        ...
        public RetryResponseListener(ShardUpsertRequest2 shardUpsertRequest) {
